...
use crate::i18n::I18n;
use crate::ingestion::{IngestionService, hash::compute_content_hash};
use crate::ollama::{
...
pub struct SeneschalService {
    ...
    processing_cancellation_tokens: Arc<DashMap<String, CancellationToken>>,
}
...
impl SeneschalService {
    ...
    async fn process_document(&self, document: &Document) {
        ...
        if !chunks_to_embed.is_empty() {
            ...
            let result = self
                .search
                .index_chunks_with_progress_cancellable(&chunks_to_embed, &cancel_token_for_progress, |progress, _total| {
        ...
        // Check for cancellation before image extraction
        if self.check_cancellation(doc_id, &cancel_token).is_err() {
    ...
    /// Returns the token to be checked during processing.
    fn register_processing_token(&self, document_id: &str) -> CancellationToken {
    ...
    /// Returns true if a cancellation was triggered.
    fn cancel_document_processing(&self, document_id: &str) -> bool {
