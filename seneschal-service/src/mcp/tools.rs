...
use crate::tools::{
    CustomWorldParams, SearchFilters, TagMatch, ToolLocation, TravellerMapTool, TravellerTool,
    classify_tool,
};
...
async fn execute_internal_tool(
    ...
    match name {
        ...
        "traveller_worlds_canon_url" => execute_traveller_worlds_canon_url(state, arguments).await,
        "traveller_worlds_canon_save" => {
            execute_traveller_worlds_canon_save(state, arguments).await
        }
        "traveller_worlds_custom_url" => execute_traveller_worlds_custom_url(state, arguments),
        "traveller_worlds_custom_save" => {
            execute_traveller_worlds_custom_save(state, arguments).await
        }
        _ => Err(McpError {
            code: -32601,
...
// ==========================================
// Traveller Worlds Tool Implementations
// ==========================================

async fn execute_traveller_worlds_canon_url(
    state: &McpState,
    arguments: &serde_json::Value,
) -> Result<serde_json::Value, McpError> {
    let sector = arguments
...
async fn execute_traveller_worlds_canon_save(
    state: &McpState,
    arguments: &serde_json::Value,
) -> Result<serde_json::Value, McpError> {
    let sector = arguments
...
fn execute_traveller_worlds_custom_url(
    state: &McpState,
    arguments: &serde_json::Value,
) -> Result<serde_json::Value, McpError> {
...
async fn execute_traveller_worlds_custom_save(
    state: &McpState,
    arguments: &serde_json::Value,
) -> Result<serde_json::Value, McpError> {
    let params = CustomWorldParams {
